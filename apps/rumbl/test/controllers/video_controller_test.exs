defmodule Rumbl.VideoControllerTest do
  ...
  test "requires user authentication for all actions", %{conn: conn} do
    Enum.each([
  ...
  test "list all user's videos on index", %{conn: conn, user: user} do
    ...
    assert String.contains?(conn.resp_body, "funny cats")
    refute String.contains?(conn.resp_body, "another video")
  end
  ...
  test "renders errors when invalid", %{conn: conn} do
    ...
    assert video_count(Video) == initial_count
  end
